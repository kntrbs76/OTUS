...
resource "yandex_kubernetes_cluster" "k8s-lab" {
  ...
  service_account_id      = yandex_iam_service_account.kntrbs.id
  node_service_account_id = yandex_iam_service_account.kntrbs.id
  depends_on = [
...
resource "yandex_iam_service_account" "kntrbs" {
  name        = local.sa_name
...
resource "yandex_resourcemanager_folder_iam_member" "editor" {
  ...
  folder_id = yandex_resourcemanager_folder.folders["labfolder"].id
  role      = "editor"
  member    = "serviceAccount:${yandex_iam_service_account.kntrbs.id}"
}
/*
